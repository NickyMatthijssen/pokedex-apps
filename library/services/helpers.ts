import { FORMATTED_NAMES, STAT_NAMES } from "./constants";
...
export function formatPokmonName(name: string): string {
  if (FORMATTED_NAMES[name]) {
  ...
  return name.replace(name.charAt(0), firstCharacter);
}
